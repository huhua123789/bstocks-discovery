...
### 3.4 Web3 钱包证券分类与 Ondo 隐藏策略

APK 内置的 Web3 钱包市场配置（`defi_public_1.json`，获取于 2026-05-31 11:28 UTC+8）暴露了 Binance 对代币化证券的完整产品规划。以下分类层级全部来自公开配置原文：

```
Web3 钱包市场 Tab 结构（APK 内部 key）
├── TRENDING (t=10)          ← 链分类：BSC/Solana/Ethereum/Base/Sonic
├── TOP_SEARCH (t=11)        ← 同上
├── ALPHA (t=20)             ← 早期代币专区
│   ├── points+              ← 积分/Boost 项目
│   └── securities (t=12)    ← ck=["hideOndoAssets"] ⚡
├── STOCK (t=40)             ← 整个板块 ck=["hideOndoAssets"] ⚡
│   ├── Top Gainers          ← 以下 10 个子分类全部继承父级 ck
│   ├── AI  ·  Energy  ·  Metals
│   ├── China Concepts  ·  Magnificent Seven
│   ├── Crypto Securities  ·  ETF
│   ├── Top Tech Securities  ·  Buffett Portfolio
├── PRE_IPO (t=43)           ← ck=["hideOndoAssets"] ⚡
├── BN_CN_MEME  ·  Hook Summer  ·  x402
└── PERPS (t=50)
    └── Stock                ← 合约衍生品（无 ck 限制）
```

**关键发现：STOCK (t=40) 内部 key ≠ 线上 UI 名称**

APK 打包于 2026-05-29，线上产品于 2026-06-01 上线。研究时发现：
- APK 配置 key 为 `STOCK`（t=40）
- 线上产品 UI 显示名为 **`Tradfi`**（或中文"证券"）
- 该板块在 App 内与 Alpha 平级，并非 Alpha 的子分类

即 APK 开发阶段的内部代号 `STOCK` 最终对外命名为 `Tradfi`。这一修正在 6/1 上线后得到验证。

**三个板块同时默认隐藏 Ondo**：Alpha 区的 `securities`、STOCK/Tradfi 专区（含全部 10 个子分类）、`PRE_IPO` 板块，全部配置了 `ck: ["hideOndoAssets"]`。这不是单个入口的配置，是贯穿整个新产品的全局策略。

**"确定"与"推测"的区分**：

| 层级 | 内容 | 性质 |
|---|---|---|
| **确定** | `hideOndoAssets` 配置原文存在，挂在三个板块上 | 公开 API 返回数据 |
| **确定** | 配置 key `STOCK` = 线上 UI 名称 `Tradfi/证券` | APK 配置 + 6/1 线上产品验证 |
| **确定** | `{TICKER}B` 命名规则（ADIB） | 公开 API + 股票 API 交叉验证 |
| **推测** | `hideOndoAssets` 的目的是隐藏 Ondo（`on` 后缀），为 bStocks（`B` 后缀）腾出展示空间 | 配置证据 + 时序逻辑（2 月合作 → 5/29 APK 埋码 → 6/1 上线），但代码未写明动机 |

**来源**：`work/api/defi_public_1.json`（Binance Web3 公开配置，获取于 2026-05-31 11:28 UTC+8）
