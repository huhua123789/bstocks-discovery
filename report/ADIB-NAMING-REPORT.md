...
APK 内置的 Web3 钱包市场配置（`defi_public_1.json`）暴露了 Binance 对代币化证券的完整产品规划。以下分类层级全部来自公开配置原文：
...
**三个板块同时默认隐藏 Ondo**：Alpha 区的 `securities`、独立的 `STOCK` 专区、`PRE_IPO` 板块，全部配置了 `ck: ["hideOndoAssets"]`。这不是单个入口的配置，是贯穿整个新产品的全局策略。

**"确定"与"推测"的区分**：

- **确定**：`hideOndoAssets` 配置原文存在，挂在三个证券相关板块上，来自公开 API 返回
- **推测**：藏掉 Ondo 是为了给 bStocks 让路（代码只说了"藏"，没写"为什么"）
